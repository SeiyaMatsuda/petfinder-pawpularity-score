...
class CONFIG:
    ...
    NO_LABEL_DATA_PATH = Path('../input/petfinder-adoption-prediction')
    PRETRAINED_DIR = Path('../output/29')
    OUTPUT_DIR = Path('../output/30')
...
path = list(map(str, list(CONFIG.NO_LABEL_DATA_PATH.glob('train_images/*-1.jpg'))))+ list(map(str, list(CONFIG.NO_LABEL_DATA_PATH.glob('test_images/*-1.jpg'))))
Id = list(map(lambda x:os.path.basename(x).split('.')[0],path))
unlabel_train_df  = pd.DataFrame({'Id':Id, 'path':path})
unlabel_train_df['image_size'] = unlabel_train_df['path'].apply(lambda image_id : Image.open(image_id).size)
unlabel_train_df['width'] = unlabel_train_df['image_size'].apply(lambda x: x[0])
unlabel_train_df['height'] = unlabel_train_df['image_size'].apply(lambda x: x[1])

if CONFIG.MASK:
    train_df['mask_path'] = train_df['Id'].map(lambda x:str(CONFIG.DATA_PATH/'train_U2NET'/x)+'.jpg')
    test_df['mask_path'] = test_df['Id'].map(lambda x:str(CONFIG.DATA_PATH/'test_U2NET'/x)+'.jpg')
    mask_path = list(map(str, list(CONFIG.NO_LABEL_DATA_PATH.glob('train_U2NET/*-1.jpg'))))+ list(map(str, list(CONFIG.NO_LABEL_DATA_PATH.glob('test_U2NET/*-1.jpg'))))
    unlabel_train_df['mask_path'] = mask_path
train_df.head()
...
def train_fn(train_loader, model, criterion, optimizer, scheduler, batch_size, epoch, device):
    ...
    for iter, data in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if CONFIG.MIX_UP and (torch.rand(1)[0] < 0.5):
...
def give_label_fn(unlabeld_train_loader, model, device):
    start = end = time.time()
    losses = AverageMeter()
    ...
    for iter, data in tqdm(enumerate(unlabeld_train_loader), total=len(unlabeld_train_loader)):
        img, target = data['image'], data['targets']
        img = img.to(device)
        target = target.to(device)
        batch_size = target.size(0)
    ...
    pseudo_label = np.concatenate(preds) * 100
    return pseudo_label
...
def train_loop(train, unlabeld_train, fold_):
    ...
    if torch.cuda.device_count()>1:
        ...
        model.load_state_dict(torch.load(CONFIG.PRETRAINED_DIR / f"{CONFIG.MODEL_NAME}_{fold_}_best.pth")["model"])
    else:
        model.load_state_dict(fix_model_state_dict(torch.load(CONFIG.PRETRAINED_DIR / f"{CONFIG.MODEL_NAME}_{fold_}_best.pth")["model"]))
    dense_features = [
    ...
    unlabeled_train_folds = unlabeld_train.reset_index(drop=True)
    
    train_dataset = PawpularDataset(
        train_folds, targets=train_folds['Pawpularity']/100,
        augmentations=train_aug, mask=CONFIG.MASK
    )
    unlabeled_train_dataset = PawpularDataset(
        unlabeled_train_folds, targets=None,
    ...
    unlabeld_train_loader = DataLoader(
    ...
    for epoch in range(CONFIG.epoch):
        ...
        # give label to unlabled data
        pseudo_label = give_label_fn(unlabeld_train_loader, model,CONFIG.device)
        # re_train
        unlabeled_train_folds['Pawpularity'] = pseudo_label
        new_train_folds = pd.concat([train_folds, unlabeled_train_folds], axis=0).reset_index(drop=True)

        new_train_dataset = PawpularDataset(
        new_train_folds, targets=new_train_folds['Pawpularity']/100,
        augmentations=train_aug, mask=CONFIG.MASK
        )
        ...
        avg_loss = train_fn(new_train_loader, model, criterion, optimizer, scheduler, CONFIG.batch_size, epoch, CONFIG.device)
        ...
        LOGGER.info(
            f"pseudo label　Epoch {epoch+1} - avg_train_loss: {avg_loss:.4f}  avg_val_loss: {avg_val_loss:.4f}  RMSE: {score}  lr: {scheduler.get_lr()[0]:.8f} time: {elapsed:.0f}s"
        )
...
def main():
    ...
    if CONFIG.training_step:
        for fold in range(CONFIG.fold):
            _oof_df = train_loop(train_df, unlabel_train_df, fold)
